...
# exponential decay
indicators$rev_TFC <- 100- indicators$TotalFoliarCover
TotalFoliarCover_expon <- lm(indicators$logh ~ indicators$rev_TFC)

plot(indicators$logh ~ indicators$rev_TFC)

crPlot(TotalFoliarCover_expon, variable = "indicators$rev_TFC")
...
# log(y) = (0.058398 *(x)) + -2.193948 )

#exp(0.058398 *(x)) + -2.193948 )
...
eq <- expression(paste("Linear Regression\nLn (y) = 0.058398x -2.193948 "))

# solving Y for every X to get our curve overlaying the scatterplots
Xs <- c(0:100)
line <- as.data.frame(Xs)
min_line <- line %>% mutate(Q = exp((0.058398*Xs)-2.193948 ))



# solving for Y (Q) at 43

exp((0.058398*43)-2.193948 )
...
fit <- lm(hflux_md ~ rev_TFC, data=indicators)


#fit piecewise regression model to original model, estimating a breakpoint at x=450
segmented.fit <- segmented(fit, seg.Z = ~rev_TFC, psi=45)
...
# If x ≤ BG_BP:  y = -13.2087 + 0.3383 *(x)

# If x > BG_BP:  y = -13.2087  + 0.3383 *(BG_BP) + (0.3383 +7.0636 )*(x-BG_BP)

# solving Y for every X to get our curve overlaying the scatterplots

Xs <- unique(indicators$TotalFoliarCover)
line <- as.data.frame(Xs)
seg_line <- line %>% mutate(Q = ifelse(Xs <= TFC_BP, -13.2087  +  0.3383*(Xs),
                                       -13.2087 + 0.3383 *(TFC_BP) + (0.3383 +7.0636 )*(Xs-TFC_BP)))
# solving for the AERO bench 
-13.2087  +  0.3383*( 80.726)
#14.10091
...
TFC_regressions <- ggplot() + geom_point(data = indicators, aes(x = rev_TFC, y = hflux_md), size=1)+
  ...
  theme(text=element_text(size=12),
        ...
        axis.line = element_line(colour = "black"),
        legend.position = "none") +
  geom_segment(aes(x=80.726, y=14.10091, xend=80.726, yend=195), linetype="dashed",col = "royalblue3")+
  geom_label(aes(x=80.726, y=194,  label = "Segmented"),
             label.size = NA, color = "royalblue3")+
  geom_segment(aes(x=43, y=1.373231, xend=43, yend=195), linetype="dashed",col = "indianred4")+
  geom_label(aes(x=43, y=194,  label = "Linear"),
             label.size = NA, color = "indianred4")+
  geom_line(data = seg_line, aes(x = Xs, y = Q), color = "royalblue3", size = 0.7)+
  ylim(0,400)+xlim(0,100.1)+
  scale_x_continuous(labels=c("0"="100", "25"= "75",
                              "50"="50", "75"="25", "100"="0"))
